 ...
 * 说明：
 */
@Service("roleService") public class RoleServiceImpl extends BaseService<Role>
    implements RoleService {

    @Autowired RoleMapper roleMapper;

    @Override public int insertRole(Role role) {
        return getMapper().insert(role);
    }

    @Override public int updateRole(Role role) {
    ...
    @Override public int deleteRole(int id) {
        return getMapper().deleteByPrimaryKey(id);
    }

    @Override public int insertRoleFunction(int roleId, int functionId) {
        return roleMapper.insertRoleFunction(roleId, functionId);
    }

    @Override public List<Map> findRoleMatchUpFunctions() {
